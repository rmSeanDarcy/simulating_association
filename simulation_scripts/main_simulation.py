...
sp_hab_carcap_log = []              # Species habitat carrying capacities(K_{i,h})
sp_grwrt_log = []                   # Species growth rates
n_knorm_log = []                    # Relative resource abundances per habitat
n_kabs_log = []                     # Absolute resource abundances per habitat
node_xyz_log = []                   # Coordinates of all habitats
sp_int_mat_log = []                 # Log of Interaction matrices
sp_rsc_prf_log = []                 # Log of all species resource preferences (theta_{i,k})
euc_rsc_prf_sim_mat_log = []        # Species euclidean distances in resource preferences matrices

cooc_cor_adj_mat_log = []           # Species correlation matrices
signif_cooc_cor_adj_mat_log = []    # Species co-ocurrence matrices (-> trimmed for correlation threshold and significance testing)

n_pop_log_log = []                  # Main output: Species abundances at final timepoint
n_pop_log_log_first = []            # Log file to check species abundances over time of first simulated run
...
param_log = []                      # Log file of settings
...
for q in range(nrep):
    ...
    sp_int_mat_log.append(pd.concat([pd.DataFrame(sp_int_mat), sp_xtra], axis = 1))
    euc_rsc_prf_sim_mat_log.append(pd.concat([pd.DataFrame(eudimat), sp_xtra], axis = 1))
    sp_rsc_prf_log.append(pd.concat([pd.DataFrame(sp_rsc_prf), sp_xtra], axis = 1))
    sp_hab_carcap_log.append(pd.concat([pd.DataFrame(sp_hab_carcap), hab_xtra], axis = 1))
    sp_grwrt_log.append(pd.concat([pd.DataFrame(sp_grwrt), hab_xtra], axis = 1))
    n_knorm_log.append(pd.concat([pd.DataFrame(n_knorm), hab_xtra], axis = 1))
    n_kabs_log.append(pd.concat([pd.DataFrame(n_kabs), hab_xtra], axis = 1))
    node_xyz_log.append(pd.concat([pd.DataFrame(node_xyz), hab_xtra], axis = 1))
    sp_indiv_data_log.append(sp_xtra)
    hab_indiv_data_log.append(pd.concat([pd.DataFrame(n_nbr_num), hab_xtra], axis = 1))
    rsc_indiv_data_log.append(rsc_xtra)
    n_pop_log_log.append(pd.concat([pd.DataFrame(n_pop_log[-1]), hab_xtra], axis = 1))
...
df = pd.DataFrame(param_log, columns = headers)
df.to_csv("param_log.csv")
...
n_pop_log_log = pd.concat(n_pop_log_log)
