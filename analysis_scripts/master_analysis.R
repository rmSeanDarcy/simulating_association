...
master_folder <- '/run/user/1000/gvfs/smb-share:domain=share,server=share.univie.ac.at,share=ter,user=seand93/PROJECTS/SomSOM/Step_III/SAMSARA'
level1_folders <- list.dirs(path = './simulation_data/fig5', full.names = TRUE, recursive = FALSE)
level2_folders <- unlist(lapply(level1_folders, function(f) {
  list.dirs(path = f, full.names = TRUE, recursive = FALSE)
}))

###########################################################################################
### Loop through each second-level folder
folder <- level2_folders[1]
for (folder in level2_folders) {
  ...
  ##############################################################################################################################
  ### The End
  print(paste("We done did it for:", folder))
}
