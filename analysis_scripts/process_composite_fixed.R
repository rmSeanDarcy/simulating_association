...
###########################################################################################
##### 1. This is where we control how many habitats in which cubes are sampled        #####
###########################################################################################
#scale_com <- get_scale_coms_controlled(sdim_vec, nxyz, npop, sp_nms, rsc_nms, rsc_num, r_num)
scale_com <- get_scale_coms_controlled(sdim_vec, nxyz, npop, sp_nms, rsc_nms, rsc_num, r_num)
coord_cub <- scale_com[[1]]
npop_cub <- scale_com[[2]]
kabs_cub <- scale_com[[3]]
write.csv(npop_cub, './npop_cub_all.csv')
write.csv(kabs_cub, './kabs_cub_all.csv')
write.csv(coord_cub, './coord_cub_all.csv')

###########################################################################################
### 1.1. Generating data for the same number of habitats per cube
out <- get_equal_cubsums_fullyequal(npop_cub,kabs_cub,sp_num,rsc_num,coord_cub)
npop_cubd_sums <- out[[1]]
kabs_cubd_sums <- out[[2]]
write.csv(npop_cubd_sums, './npop_cubd_sums.csv')
write.csv(kabs_cubd_sums, './kabs_cubd_sums.csv')
npop_comp <- npop_cubd_sums
kabs_comp <- kabs_cubd_sums
coords_comp <- coord_cub
coords_comp <- cbind(coords_comp[,1:3],coords_comp[,5],0,'H',coords_comp[,4],coords_comp[,6])
colnames(coords_comp) <- c('x_coord', 'y_coord', 'z_coord', 'repl', 'err', 'n_nms', 'smpl_cubs_idx','d')
